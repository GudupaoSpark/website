...
<header class="flex items-center justify-between px-4 py-2 bg-transparent fixed top-0 left-0 right-0 z-50 transition-all duration-300" transition:persist={'header-bar'}>
  <!-- Left section -->
  <div class="flex-shrink-0 w-10 md:w-auto">
    ...
    <a href={`/${lang || 'en'}`} class="hidden md:flex items-center justify-center">
      <img src="/home/logo.png" alt="Logo" class="w-40 h-24 object-contain logo-light" draggable={false} on:contextmenu|preventDefault={() => {}} />
      <img src="/home/logo-dark.png" alt="Logo" class="w-40 h-24 object-contain logo-dark" draggable={false} on:contextmenu|preventDefault={() => {}} />
    </a>
  </div>
  ...
  <div class="flex-1 flex justify-center">
    ...
    <a href={`/${lang || 'en'}`} class="md:hidden flex items-center justify-center">
      <img src="/home/logo-mobile.png" alt="Logo" class="w-32 h-16 object-contain logo-light" draggable={false} on:contextmenu|preventDefault={() => {}} />
      <img src="/home/logo-mobile-dark.png" alt="Logo" class="w-32 h-16 object-contain logo-dark" draggable={false} on:contextmenu|preventDefault={() => {}} />
    </a>
    <!-- Desktop Navigation -->
    <nav class="hidden md:flex items-center justify-center px-12 py-3 rounded-full glass">
  ...
  <!-- Right section -->
  <div class="flex-shrink-0 w-10 md:w-auto flex justify-end">
...
<div id="mobile-menu" class="md:hidden fixed top-0 left-0 w-full h-full bg-black/50 backdrop-blur-sm z-50 hidden">
  <div class="flex flex-col items-center justify-center h-full space-y-8 relative">
    <button id="close-menu" class="absolute top-8 right-8 w-10 h-10 flex items-center justify-center rounded-lg glass hover:bg-white/30 dark:hover:bg-slate-700/50 transition-all duration-300 text-text-primary-light dark:text-text-primary-dark close-menu-icon">
      <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
